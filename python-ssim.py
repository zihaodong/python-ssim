...
for path, d, filelist in g:
    for filename in sorted(filelist):
        print(filename)
        if filename.startswith('000') and filename.endswith('png'):
            img1.append(f1+'/'+filename)
            img2.append(f2+'/'+filename)

assert len(img1)==len(img2)
ssim = 0.0
for i in range (len(img1)):
    img_mat_1 = imread(img1[i], flatten=True)
    img_mat_2 = imread(img2[i], flatten=True)
    # img_mat_2 = cv2.resize(img_mat_2, (512, 512), interpolation=cv2.INTER_AREA)

    ssim = ssim + compute_ssim(img_mat_1,img_mat_2)

print ('ssim:'+str(ssim/len(img2)))
